...
def stats(exp, ds, var, period):
    ...
    if not fn_lst.is_file():
        ...
        ds_var_gr.min().to_netcdf(dir_data / f'{exp}_{var}_{period[0]}_{period[1]}__monthlymin.nc')
        ...
        ds_var_gr.min().to_netcdf(dir_data / f'{exp}_{var}_{period[0]}_{period[1]}__seasonalmmin.nc')
...
if data_cordex:
    for mii, model in enumerate(models):
        for exp in ['historical', 'rcp85_mid', 'rcp85_end']:
            ...
            if isinstance(ds_cordex_hist.indexes['time'], xr.CFTimeIndex) and model != 7 and model != 17:
                print('################')
                print(model)
